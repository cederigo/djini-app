...
import { fulfilled, isIdea } from '../lib/wishUtil'
...
function eventCategory(wish) {
  return isIdea(wish) ? 'idea' : 'wish'
}
...
export function saveWish(wish: Record<Wish>, source: string = 'details') {
  return dispatch => {
    ...
    return parseWish
      ...
      .then(data => {
        if (wish.id) {
          trackEvent(eventCategory(wish), 'updated', { label: wish.title })
          dispatch(wishUpdated(data, source))
        } else {
          trackEvent(eventCategory(wish), 'created', { label: wish.title })
...
export function deleteWish(wish: Wish, source: ?string = 'swipe') {
  return dispatch => {
    ...
    function deleteConfirmed() {
      ParseWish.createWithoutData(wish.id)
        .destroy()
        .then(() => {
          trackEvent(eventCategory(wish), 'deleted', { label: wish.title })
    ...
    Alert.alert(
      ...
      [{ text: 'Abbrechen', style: 'cancel' }, { text: 'Ja', onPress: deleteConfirmed }]
    )
  }
}
...
export function fulfillWish(wishRecord, contact) {
  return dispatch => {
    ...
    const onConfirmed = () => {
      ...
      trackEvent(eventCategory(wish), 'fulfilled', { label: wish.title })
    }
    // Show Note edit
    Actions.noteDialog({
      dispatch,
...
export function unfulfillWish(wish) {
  return dispatch => {
    ...
    trackEvent(eventCategory(wish), 'unfulfilled', { label: wish.title })
  }
}
...
export function copyWish(wish: Record<Wish>, user: User) {
  return dispatch => {
    ...
    trackEvent(eventCategory(wish), 'copied', { label: wish.title })

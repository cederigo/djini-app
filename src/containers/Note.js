...
class Note extends Component {
    ...
    if (state === 'wish-deleted') {
      ...
      value = (
        <TouchableOpacity style={styles.value} onPress={() => this.openWish(note)}>
          <DjiniText style={[styles.value, styles.wishText]} numberOfLines={1}>
            {wish.title}
          </DjiniText>
        </TouchableOpacity>
      )
  ...
  save() {
    ...
    dispatch(
      saveNote({ ...note, title: title.value, dueDate: dueDate.value, comment: comment.value })
    )
    onSave(this)
  }
    ...
    return (
      <View style={styles.container}>
        <AppBar
          ...
          textStyle="dark"
          showBackButton={true}
          backButtonText={edit ? 'Abbrechen' : undefined}
        ...
        >
          {edit ? (
            ...
            <ActionButton textStyle="dark" text="Bearbeiten" onPress={() => this.edit()} />
          ) : (
            undefined
          )}
        </AppBar>
      ...
      <ScrollView keyboardShouldPersistTaps="always" style={styles.container}>
        <View style={[styles.row, styles.titleField]}>
          <DjiniIcon
            ...
            size={60}
            name={type === 'reminder' ? 'cake' : 'giftdarkblue'}
          />
          {fields.title.editable && edit ? (
            ...
            <DjiniText style={[styles.value, styles.title]} numberOfLines={2}>
              {title}
            </DjiniText>
          )}
        </View>
        ...
        >
          ...
          {fields.dueDate.editable && edit ? (
            <DateInput
              autoFocus={!fields.title.editable}
        ...
        {wish && !isNew ? this.renderWishField(this.props.note) : undefined}
          ...
          <View style={[styles.row, styles.field]}>
            ...
            {fields.comment.editable && edit ? (
              <DjiniTextInput
                ...
                onChangeText={val => this.onValueChange('comment', val)}
              />
            ) : (
              <DjiniText style={styles.value} numberOfLines={3}>
                {comment}

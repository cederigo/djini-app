...
describe('./lib/pushNotificationFactory', () => {
  describe('getNotesNotifications()', () => {
    ...
    it('computes notifications correctly (reminder notes)', () => {
      ...
      expect(result[0].alertBody).toBe(
        `Denk daran, dass Alfonso und Thierry in 14 Tagen Geburtstag haben`
      )
      expect(result[0].fireDate).toBe(
        moment(d1)
          .subtract(14, 'days')
          .hour(9)
          .minute(0)
      ...
      expect(result[1].alertBody).toBe(
        `Alfonso und Thierry haben am ${moment(d1).format(
          'Do MMMM'
        )} Geburtstag. Weißt du schon, was du schenken wirst?`
      )
      expect(result[1].fireDate).toBe(
        moment(d1)
          .subtract(7, 'days')
          .hour(9)
          .minute(0)
      ...
      expect(result[2].alertBody).toBe(
        `Alfonso und Thierry haben heute Geburtstag. Gratulieren nicht vergessen!`
      )
      expect(result[2].fireDate).toBe(
        moment(d1)
          .hour(9)
          .minute(0)
          .valueOf()
    ...
    it('computes notifications correctly (task notes)', () => {
      const d1 = new Date()
      const notes = [getTaskNote({ dueDate: d1 }, 'Hannes')]
      let result = getNotesNotifications(notes)

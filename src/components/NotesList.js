...
class NotesList extends Component {
  ...
  getDescription(note) {
    ...
    return (
      note.contact.name + (note.dueDate ? `, ${parseDate(note.dueDate).format('Do MMMM')}` : '')
    )
  }
  ...
  renderRow(note) {
    ...
    return (
      ...
      >
        {!note.dueDate || note.state === 'wish-deleted' ? (
          <ListRowIcon style={styles.incomplete} name="error-outline" />
        ) : (
          undefined
        )}
        {icon}
  ...
  renderEmptyList() {
    return (
      <DjiniText style={styles.emptyList}>
        Favorisiere Kontakte und erfülle Wünsche oder eigene Ideen. Djini erinnert dich an
        Geburtstage von favorisierten Kontakten und an Geschenke, die du machen willst.
      </DjiniText>
    )
  }
